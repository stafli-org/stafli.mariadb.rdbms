...
RUN printf "Updading MariaDB configuration...\n" && \
    ...
    perl -0p -i -e "s># for more write intensive setups, set to DEMAND or OFF>query_cache_min_res_unit\t= ${app_mariadb_query_cache_min_res_unit}\n# for more write intensive setups, set to DEMAND or OFF>" ${file} && \
    ...
    perl -0p -i -e "s>.*innodb_log_buffer_size>innodb_buffer_pool_instances\t= ${app_mariadb_innodb_buffer_pool_instances}\ninnodb_log_buffer_size>" ${file} && \
